...
\section{The Client: Scikit-Learn}
\label{scikit-learn}
...
If he/she wants to publish a performance metric for a task which they performed they can do so by using the \texttt{publish()} method by passing the model used, scoring metric and the score as it's parameters as follows:
...
An utility package was introduced inside the FMLearn's client to make use of the meta-learning feature provided by auto-sklearn. This utility function performs the input checks, transforms the data and performs the required validations before passing on the dataframe to Auto-Sklearn's calculate meta-features method. It is available in this package via the imported method.
